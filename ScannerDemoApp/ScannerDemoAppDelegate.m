...
- (BOOL) application:(UIApplication*) application didFinishLaunchingWithOptions:(NSDictionary*) launchOptions {
    ...
    /// Initialize the ScanerAppengine.
...
- (void) applicationDidEnterBackground:(UIApplication*) application {
    ...
    if ((YES == scanner_detection) && (op_mode == DCSSDK_OPMODE_BTLE || op_mode == DCSSDK_OPMODE_ALL)) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            /// Do nothing...
            /// Waiting for online and kill background task if sign in procedure was successfull. otherwise task will be killed in expiration handler.
            int timeout = 0;
...
- (void) createDir:(NSString*) rootDir {
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:rootDir isDirectory:&isDirectory] || !isDirectory) {
        ...
        [[NSFileManager defaultManager] createDirectoryAtPath: rootDir withIntermediateDirectories: YES attributes: attr error: &error];
        if (error) {
